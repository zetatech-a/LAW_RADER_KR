...
from ..debug_sanitize import redact_debug_html, redact_debug_text, redact_debug_url
...
# --- 진단 덤프 정화 ---
#
# 구현은 src/debug_sanitize.py 로 옮겼다. 회신사례(better_reply)와 그 진단 스크립트가
# 같은 정화를 거쳐야 하는데, 정화 로직이 두 벌로 갈라지면 한쪽만 고쳐지고 다른 쪽에서
# 비밀이 새기 때문이다. 동작은 그대로이고 여기서는 이름만 잇는다.
_redact_html = redact_debug_html
_redact_url = redact_debug_url
_redact_values = redact_debug_text
