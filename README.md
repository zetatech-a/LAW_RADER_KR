...
4. 본문이 있는 글은 **Gemini API로 3줄 요약**해 메일에 싣습니다. 원문을 앞에서 자른 발췌 대신 핵심만 보이게 하기 위함입니다.
   ...
   - 의안 배치 호출이 **호출 계층에서 실패**하면(401·403·5xx·네트워크) 남은 배치는 호출하지 않습니다. 같은 자격증명으로 같은 서비스를 다시 부를 뿐이라 실패만 쌓이며 시간예산을 태우고 메일이 그만큼 늦어집니다. 응답 '내용'이 문제인 경우(깨진 JSON·스키마 위반 등)는 배치마다 다를 수 있으므로 멈추지 않고 다음 배치를 진행합니다.
