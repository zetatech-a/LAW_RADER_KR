...
4. 본문이 있는 글은 **Gemini API로 3줄 요약**해 메일에 싣습니다. 원문을 앞에서 자른 발췌 대신 핵심만 보이게 하기 위함입니다.
   ...
   - 금융규제포털 회신사례는 **법령해석·비조치의견서**의 질의요지·회답·이유를 `post.body` 에 담아 기존 일반 요약 경로(`min_body_chars` 이상이면 1건당 1회 호출)를 그대로 탑니다. 상세 응답이 목록의 그 글이 맞는지(제목·회신일·구분↔endpoint) 먼저 확인하고, 확인되지 않으면 본문도 첨부도 수집하지 않습니다(목록 `dataIdx`를 `lawreqIdx`/`opinionIdx`로 쓰는 매핑이 아직 라이브 확인 전이라, 가정이 틀렸을 때 다른 사건의 회답이 실리는 것을 막는 안전장치입니다). 본문은 **세 항목이 모두 있을 때만** 만듭니다 — 회답이 빠진 질의만으로 3줄 요약을 만들면 결론을 지어낸 요약이 실립니다(하나라도 없으면 본문을 비우고 어떤 항목이 없는지 warning 을 남깁니다. 첨부 수집은 계속합니다). `details` 는 채우지 않습니다 — 실제 요약 호출 경로(`_summarize_general`)는 `details` 를 보지 않고 본문 길이만 보지만, notifier 가 `details` 를 요약보다 먼저 렌더해 그 요약이 메일에 보이지 않고 집계 로그(`ai_target_count`)만 대상에서 빼 로그와 실제 호출이 어긋나기 때문입니다. 상세 주소가 확인되지 않은 구분은 본문 없이 제목·링크만 나가며, 그 글은 per-post 훅(`BaseScraper.supports_enrich`)으로 상세 수집 통계에서 제외됩니다.
